...
namespace UI
{
    public class NameMenu : IMenu
    {
        ...
        {
            Log.Information("Started order at: " + order.Store.Location);
            ...
            Log.Information($"Found {allCustomers.Count} customers in DB");

            foreach(Customer cust in allCustomers)
            {
                if(cust.Name.ToLower() == name.ToLower())
                {
                    Log.Information("Found Customer in DB: " + cust.Name);
                    match = true;
            ...
            if(match)
            {
                ...
                foreach(Customer cust in matchedCustomers)
                {
                    ...
                    {
                        Log.Information($"Logged in Customer: {cust.Name}");
                        order.Customer = cust;
                        MenuFactory.GetMenu("order").Start(order);
                    }
                    else
                    {
                        Log.Information($"Failed to log in customer: {cust.Name}");
            ...
            else 
            {
                ...
                Log.Information($"Created new Customer: {order.Customer.Name} with password {order.Customer.Password}");
